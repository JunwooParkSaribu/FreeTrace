...
def predict_multinormal(relativ_coord, alpha, k, lag):
    ...
    relativ_coord = relativ_coord[:2]
    
    log_pdf = np.sum(np.log(1/(np.sqrt(2*np.pi) * mean_std) * np.exp(-1./2 * (relativ_coord / mean_std)**2)))
    print('HEHE ', log_pdf, log_pdf2, relativ_coord[:2])
    return log_pdf, abnormal


def predict_cauchy(next_vec, prev_vec, alpha, lag, precision, prev_k):
    log_pdf = 0
    dim = 2
    abnormal = False
...
def predict_long_seq(next_path, trajectories_costs, localizations, prev_alpha, prev_k, next_times, prev_path=None, start_indice=None, last_time=-1, jump_threshold=20):
    abnormal = False
    ...
    terminal = is_terminal(next_path[-1], localizations, jump_threshold)
    ...
    if len(next_path) <= 1:
        ...
        for edge_index in range(3, len(next_path)):
            ...
            """
            input_mu = cur_coord - before_coord
            log_p0, abnormal1 = predict_multinormal(input_mu, prev_alpha, prev_k, time_gap)
            traj_cost.append(abs(log_p0))
            """
            ...
            log_p0, abnormal2 = predict_cauchy((next_coord - cur_coord), (cur_coord- before_coord), prev_alpha, time_gap, 0.5, prev_k)
            ...
            for last_node in cumulative_last_nodes:
                for cur_time in next_times[index:index+1]:
                    if last_node[0] < cur_time and last_node != source_node:
                        ...
                        print(next_times)
                        print(list(localizations.keys()))
                        print(cumulative_last_nodes)
...
def is_terminal(node, localizations, max_jump_d):
    node_t = node[0]
    node_loc_idx = node[1]
    ...
    for next_t in range(node_t + 1, node_t + TIME_FORECAST + 1):
        if next_t in localizations:
            for search_loc in localizations[next_t]:
                if len(search_loc) == 3:
    ...
    while True:
        ...
        if first_step:
            for next_path in next_paths:
                ab_index, term = predict_long_seq(next_path, trajectories_costs, localizations, 1.0, 2.0, next_times, start_indice=start_indice, last_time=last_time, jump_threshold=distribution[0])
                if term is not None:
            ...
            for next_path in next_paths:
                ...
                if prev_path is None:
                    ...
                    ab_index, term = predict_long_seq(next_path, trajectories_costs, localizations, prev_alpha, prev_k, next_times, prev_path, start_indice=start_indice, last_time=last_time, jump_threshold=distribution[0])
                if len(ab_index) > 0:
                    ab_indice[next_path] = ab_index 
                if term is not None:
                    is_terminals[next_path] = term
        ...
        if lowest_cost_traj in ab_indice:
            print("AB CUT @@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@:", lowest_cost_traj, ab_index)
        ...
        if is_terminals[lowest_cost_traj]:
            ...
            for del_node in lowest_cost_traj[1:-1]:
                next_graph.remove_node(del_node)
        """
        orphans = np.array(orphans)[np.argsort([neoud[0] for neoud in orphans])]
        for orphan_node in orphans:
            orphan_node = tuple(orphan_node)
...
def run(input_video_path:str, output_path:str, time_forecast=2, cutoff=0, jump_threshold=None, gpu_on=True, save_video=False, verbose=False, batch=False, realtime_visualization=False, return_state=0):
    ...
    loc2, loc_infos = read_localization(f'outputs/alpha_test0_loc.csv', images)
    ...
    raw_distributions = segmentation(loc2, time_steps=t_steps, lag=time_forecast)
