...
from PyQt6.QtGui import (
    ...
    QPainterPath, QDesktopServices, QTransform, QImage,
...
_VERSION = "1.6.3.0"  # Modified by Claude (claude-opus-4-7, Anthropic AI) - 2026-04-28
...
def _read_metadata_from_nd2(nd2_path):  # Modified by Claude (claude-opus-4-7, Anthropic AI) - 2026-04-28
    """Extract acquisition metadata from an ND2 file (uses `nd2` library)."""
...
def _read_metadata_from_video(path):  # Modified by Claude (claude-opus-4-7, Anthropic AI) - 2026-04-28
...
def _read_metadata_from_tif(tif_path):
    ...
    Returns dict with keys pixel_size_um / finterval_s / exposure_s / R / message.
    """
    out = {'pixel_size_um': None, 'finterval_s': None, 'exposure_s': None,
           'R': None, 'message': ''}
    try:
...
def _recompute_bg_var_and_flux_from_tif(loc_df, tif_path, R_BG=6, R_SIGNAL=3):
    ...
    Used to bypass the localiser's own bg_var / integrated_flux columns when a raw
    TIFF is alongside the loc.csv, so σ_loc matches the thesis convention.
    """
    import tifffile
...
def _compute_sigma_loc_per_spot(loc_df):
    ...
    Returns dict with per-spot arrays: sigma_loc_px, I_tot, bg_var, psf_sigma_px.
    """  # Modified by Claude (claude-opus-4-7, Anthropic AI) - 2026-04-28
    try:
    ...
    for i in range(n):
        ...
        nn = np.arange(cx_int - w // 2, cx_int + w // 2 + 1) - xc[i]
        mm = np.arange(cy_int - w // 2, cy_int + w // 2 + 1) - yc[i]
...
def _estimate_K_corrected_msd(trajs, sigma_loc_px, R, max_lag=10):
    ...
    Bounded curve_fit in (K, H) over τ=1..max_lag with SEM weighting (thesis fit).
    """  # Modified by Claude (claude-opus-4-7, Anthropic AI) - 2026-04-28
    from scipy.optimize import curve_fit
    try:
        from .module.cauchy_fit import J_var
    except Exception:
        try:
...
# Modified by Claude (claude-opus-4-7, Anthropic AI) - 2026-04-28
def _invert_H_from_rho(rho_target, Delta, R, K, sigma_loc, H_bounds=(0.02, 0.98)):
    """Bisect rho_corrected(H,...) = rho_target. Used by sensitivity panels."""
    try:
        from .module.cauchy_fit import rho_corrected
    except Exception:
        try:
...
def _run_multi_delta_scan(trajs, sigma_loc_rms_px, K, R, delta_max=None,
    ...
    """Multi-Δ corrected-Cauchy scan over Δ = 1 .. Δ_max.  # Modified by Claude (claude-opus-4-7, Anthropic AI) - 2026-04-28

    Auto-selects Δ_max as the largest Δ ≤ delta_max_cap with n_ratios ≥ min_n_ratios.
    Returns dict with deltas / H_est / n_ratios / converged / n_eff / sigma_H / delta_max.
    sigma_H is the CRLB σ_H(Δ) at the fitted Ĥ; 95% CI half-width = 1.96 σ_H.
    """
    try:
        from .module.cauchy_fit import (extract_ratios, count_ratios, fit_cauchy,
...
def _preprocess_for_adv_stats(data, pixelmicrons, framerate, cutoff_min=3, cutoff_max=99999):  # Modified by Claude (claude-opus-4-6, Anthropic AI) - 2026-03-19
    ...
    for traj_idx in traj_indices:
        ...
        for si in range(len(state_chunk_idx) - 1):
            ...
            if len(dx_consec) > 1:
                # Modified by Claude (claude-opus-4-7, Anthropic AI) - 2026-04-29
                # Suppress divide-by-zero / invalid-value warnings — the np.isfinite filter
                # below drops inf/nan rows.
    ...
    def run(self):
        try:
            ...
            for idx, (name, data, has_diff) in enumerate(self._datasets):
                ...
                # — moved here from Advanced Stats since they are also noise-free / empirical.
...
class AdvStatsWorker(QThread):  # Modified by Claude (claude-opus-4-6, Anthropic AI) - 2026-03-19
    ...
    def __init__(self, datasets, pixelmicrons, framerate, cutoff_min,
        ...
        R: motion-blur fraction (toolbar spinbox value).
        sigma_loc_rms_per_dataset: list of σ_loc_rms in pixels (or None).
        """
        super().__init__()
        self._datasets = datasets
        self._pixelmicrons = pixelmicrons
    ...
    def run(self):
        try:
            ...
            for idx, (name, data) in enumerate(self._datasets):
                ...
                # Multi-Δ corrected-Cauchy scan per state. // Modified by Claude (claude-opus-4-7, Anthropic AI) - 2026-04-28
                # Skip when MSD(τ=1) ≤ 1.5·2σ_loc² (noise-dominated; corrected fit cannot
                # recover H). Always store a noise-floor diagnostic for the user.
                multi_delta_per_state = {}
                K_est_per_state = {}
                noise_floor_per_state = {}
                ...
                if sigma_loc_rms_px is not None and np.isfinite(sigma_loc_rms_px):
                    ...
                    for st in total_states:
                        ...
                        # No fixed S/N gate — rely on K_est lower-bound rejection. // Modified by Claude (claude-opus-4-7, Anthropic AI) - 2026-04-28
                ...
                all_results.append({
                    ...
                    'noise_floor_per_state': noise_floor_per_state,
...
# ---------------------------------------------------------------------------
# ROI Canvas — spatial trajectory scatter with rectangle/ellipse/line ROIs
# ---------------------------------------------------------------------------
...
def _latex_to_unicode(text: str) -> str:  # Modified by Claude (claude-opus-4-6, Anthropic AI) - 2026-03-21
    ...
    def _convert_expr(expr: str) -> str:
        """Convert a single LaTeX expression (without $ delimiters)."""
        s = expr.strip()
        ...
        def _sup_repl(m):
            ...
            exp = m.group(2)
            translated = exp.translate(_SUP)
            # Characters that couldn't be translated stay as-is
...
class GeminiChatWorker(QThread):  # Modified by Claude (claude-opus-4-6, Anthropic AI) - 2026-03-20 17:00
    ...
    def _classify_error(msg: str) -> str:
        ...
        # Permission denied — likely policy change or free tier removed
...
class FreeTraceGUI(QMainWindow):  # Modified by Claude (claude-opus-4-6, Anthropic AI) - 2026-03-17
    ...
    _PREDEFINED_QA = [  # Modified by Claude (claude-opus-4-6, Anthropic AI) - 2026-03-21
        ...
        {
            "keywords": ["gpu", "cuda", "acceleration", "graphics card", "nvidia",
                         "speed up", "faster", "slow", "performance"],
            "question": "What does the GPU option do?",
            "answer": (
                "The GPU option (available in FreeTrace Python GUI only) uses "
                "CUDA-accelerated computation for localization and neural network inference. "
                "This significantly speeds up processing for large datasets. Requires a "
                "compatible NVIDIA GPU with CUDA installed. On Windows and macOS, "
                "FreeTrace uses pre-converted models that do not require TensorFlow."
            ),
        },
        {
            "keywords": ["save video", "export video", "video output", "movie",
                         "animation", "visualize", "overlay", "render video"],
            "question": "What does the save video option do?",
            "answer": (
                "When enabled (available in FreeTrace Python only), FreeTrace exports "
                "a video showing the original microscopy frames overlaid with detected "
                "spots and linked trajectories. There are separate options for localization "
                "video and tracking video. Useful for visual verification of results."
            ),
        },
        {
            ...
            for word in query_words:
                for kw in qa["keywords"]:
                    for kw_part in kw.split():
                        ...
                        if ratio >= 0.75:  # e.g., "threshhold" matches "threshold"
                            score += ratio
    ...
    def _build_roi_tab(self):
        ...
        # H-K scatter colored by ROI (right)
    ...
    def _render_stats_plots(self, results_list):  # Modified by Claude (claude-opus-4-6, Anthropic AI) - 2026-03-19
        ...
        if any_has_diff:
            ...
            h_section.add_widget(_make_canvas(fig))
        else:
        ...
        if any_has_diff:
            ...
            with plt.style.context(dark_style):
                ...
                if stat_lines:
                    _fill_stats_panel(ax_stats, stat_lines)
            k_section.add_widget(_make_canvas(fig))
        else:
            warn = QLabel("_diffusion.csv required for this plot.")
            warn.setStyleSheet("color:#ff8800; padding:12px;")
        ...
        with plt.style.context(dark_style):
            ...
            if need_legend:  # Modified by Claude (claude-opus-4-6, Anthropic AI) - 2026-03-19
                ax.legend(loc='upper left', bbox_to_anchor=(-0.35, 1.15))
    ...
    def _on_adv_stats_load_data(self):
        ...
        # Multi-dataset: reset σ_loc spinbox to 0 → per-dataset σ_loc used. // Modified by Claude (claude-opus-4-7, Anthropic AI) - 2026-04-29
        if n > 1:
            try:
                self._adv_stats_sigma_loc.setValue(0.0)
            except Exception:
        ...
        if n > 0:  # Modified by Claude (claude-opus-4-6, Anthropic AI) - 2026-03-26
            ...
            datasets_with_crlb = [ds for ds in self._adv_stats_datasets if ds.get('has_crlb_cols')]  # Modified by Claude (claude-opus-4-7, Anthropic AI) - 2026-04-28
    ...
    def _load_adv_stats_data_from_file(self, selected_path):
        ...
        try:
            ...
            # Falls back through "_<token>"-stripped paths so ROI/class subset
            # traces (e.g. <v>_roi_2_traces.csv, <v>_region_1_traces.csv) still find
            # the parent <v>_loc.csv. When using a parent loc file, filter rows by
            # (frame, x, y) to match only the surviving traces.
            ...
            self._adv_stats_datasets.append({
                ...
                'sigma_loc_px': sigma_loc_px,
                'sigma_loc_median_px': sigma_loc_median_px,
                'sigma_loc_rms_px': sigma_loc_rms_px,
                'I_tot_arr': I_tot_arr,
                'psf_sigma_px_arr': psf_sigma_px_arr,
                # Modified by Claude (claude-opus-4-7, Anthropic AI) - 2026-04-29
    ...
    def _on_adv_stats_read_R(self):  # Modified by Claude (claude-opus-4-7, Anthropic AI) - 2026-04-28
        ...
        if not path:
            return
        meta = _read_metadata_from_video(path)  # Modified by Claude (claude-opus-4-7, Anthropic AI) - 2026-04-28
        # All-or-nothing: only update if pixel size, frame interval, AND R all present.  // Modified by Claude (claude-opus-4-7, Anthropic AI) - 2026-04-28
        missing = []
    ...
    def _adv_stats_worker_finished(self, results_list):  # Modified by Claude (claude-opus-4-6, Anthropic AI) - 2026-03-19
        ...
        self._adv_stats_save_data_btn.setEnabled(  # Modified by Claude (claude-opus-4-7, Anthropic AI) - 2026-04-28
    ...
    def _render_adv_stats_plots(self, results_list):  # Modified by Claude (claude-opus-4-6, Anthropic AI) - 2026-03-19
        ...
        with plt.style.context(dark_style):
            ...
            # because the Gaussian-fit deviation is a useful distortion diagnostic.
            ...
            for r, st, color in _iter_colors(results_list):  # Modified by Claude (claude-opus-4-6, Anthropic AI) - 2026-03-19
                ...
                disp_panel_idx += 1
            # Section 2 addWidget moved to the end. // Modified by Claude (claude-opus-4-7, Anthropic AI) - 2026-04-29

            # ---- Noise-floor diagnostic ---- // Modified by Claude (claude-opus-4-7, Anthropic AI) - 2026-04-28
            ...
            if has_any_md:
                ...
                for r in results_list:
                    ...
                    TAU_LEVELS = [(0.05, 0.10), (0.01, 0.18)]
                    ...
                    for st in states_in_md:
                        ...
                        # Analytic bias proxy via thesis eq:ch1_Hhat_sensitivity. // Modified by Claude (claude-opus-4-7, Anthropic AI) - 2026-04-29
                        try:
                            from .module.cauchy_fit import (J_var as _Jv, J_cov as _Jc,
                    ...
                    if deltas_band is not None and len(deltas_band) > 0:
                        # Modified by Claude (claude-opus-4-7, Anthropic AI) - 2026-04-29
                        # Edge runs extend to the axis xlim margins.
                        ...
                        stat_lines.append(('  no overlay = strict trust (passes ε=0.01)',
                                            '#aaaaaa', None, None))
                        # Trust-band Δ-range lines removed (per user request). // Modified by Claude (claude-opus-4-7, Anthropic AI) - 2026-04-29
                        # End modified by Claude (claude-opus-4-7, Anthropic AI) - 2026-04-29
                    # End modified by Claude (claude-opus-4-7, Anthropic AI) - 2026-04-29
                ...
                # ---- Drift / sensitivity / reliability panels ---- // Modified by Claude (claude-opus-4-7, Anthropic AI) - 2026-04-28
                from matplotlib.colors import LogNorm as _LogNorm
                try:
                    from .module.cauchy_fit import sigma_H_crlb as _sigma_H_crlb
                except Exception:
                    try:
                ...
                for r in results_list:
                    ...
                    try:
                        from .module.cauchy_fit import (J_var as _Jv2, J_cov as _Jc2,
                    ...
                    for st in states_in_md:
                        ...
                        if finite_full.sum() < 3:
                            continue
                        # Per-Δ bias spread (analytic ∂Ĥ/∂σ²·σ²) for trust masks.
                        ...
                        for variant_id, variant_label, mask, target_section, draw_overlay in ZMAT_VARIANTS:
                            n = int(mask.sum())
                            if n < 3:
                                # Render placeholder stats-only panel for the strict case where it's empty.
                                if variant_id != 'all':
                            ...
                            # BH-FDR over the subset's own pair count.
                            FDR_Q = 0.05
                            ...
                            try:
                                ...
                                pass
                            # Trust-band brown/tan overlay only on the all-Δ matrix.
                            if draw_overlay:
                                try:
                    ...
                    for st in states_in_md:
                        ...
                        try:
                            ...
                            color = state_pal[0]
                        d_arr = deltas[finite]; H_arr = H_est[finite]; s_arr = sigma_H[finite]
                        d_adj = d_arr[1:]; drift = np.diff(H_arr)
                    ...
                    for st in states_in_md:
                        ...
                        for ci, mult in enumerate(LAMBDA_MULTS):
                            sigma_target = float(np.sqrt(mult)) * s_loc
            ...
            # Section ordering: Ĥ(Δ) first, |Z| matrix second, then 1D Disp / noise / drift / λ / Reliability. // Modified by Claude (claude-opus-4-7, Anthropic AI) - 2026-04-29
            ordered_sections = []
            if has_any_md:
    ...
    def _on_save_adv_stats_data(self):  # Modified by Claude (claude-opus-4-7, Anthropic AI) - 2026-04-28
...
with a 95% CRLB band. Flat at H≈0.5 = Brownian; monotone decrease = confinement /
non-Markov memory. K_estim (from short-lag MSD fit) and σ_loc rms appear in the
stats panel.</p>
...
<ul>
  <li>No overlay → Δ inside the strict trust band; Ĥ(Δ) is robust.</li>
  <li><b>Tan</b> (light) → borderline (passes ε=0.05 but fails ε=0.01).</li>
  <li><b>Dark brown</b> → fail-loose (Ĥ(Δ) clearly untrustworthy).</li>
</ul>

<h3>2. Pairwise |Z| significance matrix — three trust-subset variants</h3>
<p>For each pair of Δ values:
|Ĥ(Δᵢ)−Ĥ(Δⱼ)| / √(σ_H(Δᵢ)² + σ_H(Δⱼ)²). Cells inside the solid black contour are
significant under Benjamini–Hochberg FDR at q≤0.05 (≤5% of flagged cells expected
false positives). Dashed grey contours: uncorrected |z|=2 / |z|=3 (reference only;
~120 of ~2,400 pairs exceed |z|=2 by chance under H₀).</p>
...
panel 1, applied as row+column in a single 2D overlay (no double-alpha at
intersections). Cells where either Δᵢ or Δⱼ fails loose → dark brown; either
borderline → tan; both inside strict band → unshaded.</p>
...
shot-noise-limited (good); systematic offset above means residual systematic error.</p>
...
floor and grow with τ for diffusion to be detectable.</p>

<h3>5. Adjacent-Δ drift Ĥ(Δ+1)−Ĥ(Δ)</h3>
<p>First-difference of Ĥ(Δ) with ±1σ/±2σ/±3σ floors built from the joint
CRLB σ = √(σ_Ĥ(Δᵢ)² + σ_Ĥ(Δᵢ₊₁)²). Excursions outside ±2σ signal real jumps
in Ĥ at that Δ — useful to identify regime transitions. Points inside ±1σ
are inconclusive (not evidence of flatness).</p>
...
<p>Holds K = K_estim fixed and multiplies the data-derived λ_baseline by {0.1×, 0.5×,
1×, 2×, 10×} (σ_target = √m · σ_loc). 1× = data's own baseline (solid bold). When
curves agree at large Δ, Ĥ(Δ) is noise-robust there; where they fan out at small Δ,
the noise-dominated regime begins and Ĥ(Δ) is bias-prone.</p>

<h3>7. Precision map</h3>
<p>Heatmap of theoretical CRLB σ_H over (Δ, n_eff). Cyan curve = empirical n_eff(Δ)
overlaid on the contours. Reads the <i>statistical</i> precision floor on Ĥ. Note:
this map is precision only; the systematic σ_loc-bias is shown by the trust-band
overlay on panels 1 and 2, not here.</p>

<h3>Toolbar fields</h3>
<ul>
  <li><b>Load Data</b>: pick a <code>_traces.csv</code>; auto-pairs with sibling
      <code>_loc.csv</code> and TIFF (when present, recomputes <code>bg_var</code>
      / <code>integrated_flux</code> via the thesis-style annulus convention).</li>
  <li><b>σ_loc</b>: localisation precision (px) used as the noise plug-in.
      Auto-filled on data load from CRLB rms; editable.</li>
  <li><b>Pixel size (μm/px)</b>, <b>Frame rate (s)</b>: scaling for the 1D
      Displacement / TA-MSD axes (multi-Δ scan and K_estim run in raw px/frame).</li>
  <li><b>Min traj length</b>: trajectories shorter than this are dropped.</li>
  <li><b>R</b>: motion-blur fraction τ_exp/Δt ∈ [0,1]. 0 = instantaneous capture;
      1 = full-frame integration. <i>Assumes uniform illumination over the
      exposure window τ_exp; non-uniform / pulsed illumination would require
      a different blur kernel.</i></li>
  <li><b>Read metadata from video</b>: pick a TIFF/ND2 to auto-fill pixel size,
      ...
      On success, Adv Stats is automatically re-run.</li>
  <li><b>▶ Run Advanced Stats</b>: launches the background worker.</li>
  <li><b>Save Plots</b>: PNG of every rendered panel.</li>
  <li><b>Save Data</b>: per-(dataset×state) CSV of the multi-Δ scan
      ...
      K, σ_loc, R, Δ_max in the comment-line metadata header.</li>
</ul>
"""
    ...
    def _on_finished(self, success: bool, message: str):  # Modified by Claude (claude-opus-4-6, Anthropic AI) - 2026-03-19
        ...
        if success:
            ...
            video_path = self._video_path.text().strip()
    ...
    def _rebuild_roi_canvas(self):  # Modified by Claude (claude-opus-4-6, Anthropic AI) - 2026-03-26
        ...
        self._roi_canvas.set_data(combined_idx, all_traj_points,
                                  combined_mean_x, combined_mean_y)

        total = len(combined_mean_x)  # Modified by Claude (claude-opus-4-6, Anthropic AI) - 2026-03-26
